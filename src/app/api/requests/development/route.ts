...
export async function POST(request: Request) {
  ...
  if (!session) {
    logCustomerSessionDiagnostics("[make3d] /api/requests/development customer session failed", request);
  ...
  try {
    ...
    const functionDescription = getString(formData, "functionDescription");
    const budgetRange = getString(formData, "budgetRange");
    const contactPhone = getString(formData, "contactPhone");

    if (!projectName || !projectType || !functionDescription || !budgetRange || !contactPhone) {
      return NextResponse.json({ error: "请填写项目名称、项目类型、功能描述、预算范围和联系方式" }, { status: 400 });
    }
    ...
    if (attachments.length > MAX_ATTACHMENT_COUNT) {
      return NextResponse.json({ error: "一次最多上传 10 个附件" }, { status: 400 });
    }
    ...
    try {
      ...
      const created = createServiceRequest(db, {
        ...
        projectType,
        functionDescription,
        hasDrawingsOrSample: getString(formData, "hasDrawingsOrSample"),
        needsOnsiteMeasurement: getString(formData, "needsOnsiteMeasurement"),
        acceptsEveningOrWeekendContact: getString(formData, "acceptsEveningOrWeekendContact"),
