...
ROOT_DIR="/srv/make3d-worker/test-integration/phase05-h-b"
...
cleanup() {
  ...
  rm -f "${TOKEN_FILE}"
  if [[ ${status} -ne 0 ]]; then
    error "Phase05-H-B local recovery integration failed"
  fi
  exit "${status}"
}
trap cleanup EXIT

info "Checking Phase05-H-B local recovery integration environment"
if [[ ! -d "${REPO_DIR}" ]]; then
...
info "Using isolated H-B test database: ${DB_PATH}"
rm -f "${DB_PATH}"
...
info "Running Phase05-H-B recovery integration driver"
set +e
PHASE05_HB_DB_PATH="${DB_PATH}" \
PHASE05_HB_ROOT_DIR="${ROOT_DIR}" \
PHASE05_HB_SOURCE_STL="${SOURCE_STL}" \
PHASE05_HB_PROFILE_PATH="${PROFILE_PATH}" \
PHASE05_HB_SLICER_VERSION="${SLICER_VERSION}" \
SERVER_URL="${API_URL}" \
WORKER_TOKEN="${TEST_TOKEN}" \
WORKER_ID="wsl-worker-01" \
node --experimental-strip-types --experimental-specifier-resolution=node scripts/phase05-h-b-integration-driver.mjs > "${DRIVER_JSON}" 2> "${DRIVER_LOG}"
DRIVER_EXIT=$?
set -e

if [[ "${DRIVER_EXIT}" -ne 0 ]]; then
  error "Phase05-H-B driver exited with ${DRIVER_EXIT}"
  tail -120 "${DRIVER_LOG}" || true
  exit "${DRIVER_EXIT}"
fi
success "Phase05-H-B recovery integration driver passed"
...
success "Phase05-H-B local recovery integration passed"
