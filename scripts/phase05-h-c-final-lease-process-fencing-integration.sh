...
ROOT_DIR="/srv/make3d-worker/test-integration/phase05-h-c-final"
...
cleanup() {
  ...
  rm -f "${TOKEN_FILE}"
  if [[ ${status} -ne 0 ]]; then
    error "Phase05-H-C final lease process fencing integration failed"
  fi
  exit "${status}"
}
trap cleanup EXIT

info "Checking Phase05-H-C final integration environment"
if [[ ! -d "${REPO_DIR}" ]]; then
...
rm -rf "${ROOT_DIR}/files" "${ROOT_DIR}/processing" "${ROOT_DIR}/results" "${ROOT_DIR}/failed" "${ROOT_DIR}/db"
...
info "Using isolated H-C final test database: ${DB_PATH}"
rm -f "${DB_PATH}"
rm -f "${NEXT_LOG}" "${DRIVER_LOG}" "${DRIVER_JSON}"
...
info "Running Phase05-H-C final lease process fencing driver"
set +e
PHASE05_HC_FINAL_DB_PATH="${DB_PATH}" \
PHASE05_HC_FINAL_ROOT_DIR="${ROOT_DIR}" \
PHASE05_HC_FINAL_SOURCE_STL="${SOURCE_STL}" \
PHASE05_HC_FINAL_PROFILE_PATH="${PROFILE_PATH}" \
PHASE05_HC_FINAL_SLICER_VERSION="${SLICER_VERSION}" \
SERVER_URL="${API_URL}" \
WORKER_TOKEN="${TEST_TOKEN}" \
WORKER_ID="wsl-worker-01" \
node --experimental-strip-types --experimental-specifier-resolution=node scripts/phase05-h-c-final-lease-process-fencing-driver.mjs > "${DRIVER_JSON}" 2> "${DRIVER_LOG}"
DRIVER_EXIT=$?
set -e

if [[ "${DRIVER_EXIT}" -ne 0 ]]; then
  error "Phase05-H-C final driver exited with ${DRIVER_EXIT}"
  tail -200 "${DRIVER_LOG}" || true
  exit "${DRIVER_EXIT}"
fi
success "Phase05-H-C final driver passed"
...
success "Phase05-H-C final lease process fencing integration passed"
